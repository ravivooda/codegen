...
def generateFileForJSONArray(jsonArray, nameOfFile):
	...
	for element in jsonArray:
		for dictObject, value in element.items():
...
def generateFileForJSONObject(jsonObject: dict, nameOfFile):
	classProperties = []
	parsingStrings = []
	importStrings = []
...
def generateFilesForJSONFromURL(urlString, name):
	print('''
#######################################
Fetching Response from %s
#######################################

	''' % urlString)
	response = requests.get(urlString)
	generateFileForJSONObject(json.loads(response.text), name)
...
if __name__ == "__main__":
	...
	# jsonURL = 'https://query2.finance.yahoo.com/v7/finance/options/aprn'
	jsonURL = "https://api.plos.org/search?q=title:DNA"
	if len(sys.argv) > 1:
		jsonURL = sys.argv[1]
	generateFilesForJSONFromURL(jsonURL, "Calls")
